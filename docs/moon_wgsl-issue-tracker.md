...
| `WGSL-011` | Upstream parity gap | Namespace-style qualified module references such as the upstream `rusty_imports` case still rely on paths like `a::x::square(...)` without an explicit `#import`, which the current source-level resolver does not model. | `BLOCKED` | Supporting this cleanly would require a new resolution pass for fully qualified module-path expressions, not just import directives. No regression test was ported yet; keep this blocked until that model is designed. |
